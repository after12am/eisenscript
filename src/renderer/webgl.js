var WebGLRenderer = function(w, h) {
    ...
    this.camera = new THREE.PerspectiveCamera( this.fov, this.w / this.h, this.near, this.far );
...
WebGLRenderer.prototype.build = function(objects) {
  if (!objects) return;
  // build the object in accordance with design specifications and place on stage
  for (var i = 0, l = objects.length; i < l; i++) {
    switch (objects[i].type) {
      case Type.Background: this.setBackgroundColor(objects[i].color); break;
      case Type.Primitive: this.addPrimitive(objects[i]); break;
    }
  }
}

WebGLRenderer.prototype.setBackgroundColor = function(hex, alpha) {
    alpha = alpha || 1;
    this.renderer.setClearColor( new THREE.Color( hex ), alpha );
};


WebGLRenderer.prototype.removePrimitives = function() {
    var that = this;
    this.cubes.forEach(function(mesh) { that.group.remove(mesh); });
    this.cubes = [];
};

WebGLRenderer.prototype.addPrimitive = function(parameters) {
...
WebGLRenderer.prototype.updateRenderSize = function(w, h) {
    this.w = w;
    this.h = h;
};

// WebGLRenderer.prototype.updateProjectionMatrix = function() {
//     this.camera.projectionMatrix = THREE.Matrix4.makePerspective( this.fov, this.w / this.h, 1, 3000 );
// }
...
WebGLRenderer.prototype.resetCamera = function() {
...
WebGLRenderer.prototype.saveImage = function() {
